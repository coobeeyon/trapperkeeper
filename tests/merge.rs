...
fn prime_outputs_orphan_mode_text() {
    ...
    assert!(
        out.contains("default Trapper Keeper layout") || out.contains("default layout"),
...
fn prime_outputs_in_tree_mode_text() {
    ...
    assert!(
        !out.contains("`trapperkeeper` branch") && !out.contains("`trapperkeeper` orphan branch"),
...
fn assert_sync_discipline_documented(prime_out: &str) {
    ...
    assert!(
        prime_out
            .to_ascii_lowercase()
            .contains("before reading or editing"),
    ...
    assert!(
        prime_out.contains("conflict marker")
            || prime_out.contains("conflict-marker")
            || prime_out.contains("<<<<<<<"),
