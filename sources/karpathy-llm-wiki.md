...
- **Download images locally.** In Obsidian Settings, set "Attachment folder path" to a fixed directory (e.g. `raw/assets/`). Then bind "Download attachments for current file" to a hotkey. After clipping an article, hit the hotkey and all images get downloaded to local disk.
